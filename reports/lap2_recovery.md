...
Lap 1's TF-IDF model missed 53 real unfair clauses at its recall-first operating point (the dataset's non-clause header stubs are already removed upstream by `data.py`'s cleaning, so every item here is a real clause). The Lap 1 error analysis argued these residual misses were SEMANTIC - waiver/responsibility clauses unfair by legal meaning, not vocabulary. If that diagnosis was right, a legal-domain encoder should recover them.

**Result: legal-bert recovers 28 of 53 real missed clauses (53%).**
...
- (`potentially_unfair`) Spam is an issue of consent, not content.
...
- (`clearly_unfair`) Zynga’s mission is to connect the world through games.
- (`clearly_unfair`) “Community Rules” means the rules of conduct that govern your interaction with our Services and other players, which can be found here.
...
- (`clearly_unfair`) THE SERVICE IS PROVIDED ON AN "AS IS" AND "AS AVAILABLE" BASIS.
- (`potentially_unfair`) Use of the materials by the U.S. Government constitutes acknowledgment of our proprietary rights in them.
- (`potentially_unfair`) The failure of Weebly to exercise or enforce any right or provision of these Terms shall not constitute a waiver of such right or provision.
- (`potentially_unfair`) Further, more than one person's claims may not be consolidated under any circumstances, in any form of any class or representative proceeding or otherwise.